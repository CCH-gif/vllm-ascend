 ...
 *
 * Why: the python custom_op impls register at the python dispatch key, which
 * outranks PrivateUse1.  torch_npu's aclgraph therefore does not treat them as
 * pure device nodes -- it re-dispatches them through the python dispatcher on
 * every graph replay.  A probe at the punica binding site measured ~496 host
 * calls/s sustained through a c=1 decode plateau, against 0/s for the AscendC
 * ops.  Registering the same schemas on the PrivateUse1 dispatch key ONLY
 * (exactly how vllm_ascend_C.so registers its AscendC ops, see
 * csrc/torch_binding.cpp:2493) makes them torch-native: aclgraph captures
 * their kernel launches and decode replays them with no python on the step
 * path.
 *
...
// NOTE (2026-09-15): the Python-key registration that used to live here is
// deliberately REMOVED.  It was added on the theory that "the PrivateUse1 key
// is not in the dispatch key set ... so LoRA silently no-ops".  That theory is
// falsified by the AscendC path: csrc/torch_binding.cpp:2493 registers
// _C_ascend's LoRA ops at kPrivateUse1 ONLY, and a punica-level probe measures
// ZERO host calls across an entire decode benchmark -- i.e. PrivateUse1-only
// dispatch is captured into the aclgraph as device nodes and replayed with no
// python.
//
// Registering a second impl on the Python key makes things strictly worse: the
// Python key outranks PrivateUse1, so the op stops being a pure device node and
// torch_npu re-dispatches it through the python dispatcher on every replay.
// Measured on the triton path: ~496 host calls/s during a c=1 decode plateau
// (base: 0), which is the whole of the low-concurrency regression.
