...
#include "grouped_matmul_swiglu_bf16/grouped_matmul_swiglu_bf16_torch_adpt.h"
...
void enqueue_device_print(std::unique_ptr<DevicePrintPayload> payload,
                          aclrtStream stream)
{
    // aclrtLaunchHostFunc not available in CANN < 9.0 — skip callback
    (void)payload;
    (void)stream;
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.def(
        "grouped_matmul_swiglu_bf16(Tensor x, Tensor[] weight, Tensor group_list) -> Tensor"
    );
    ops.impl("grouped_matmul_swiglu_bf16", torch::kPrivateUse1, &vllm_ascend::grouped_matmul_swiglu_bf16);
